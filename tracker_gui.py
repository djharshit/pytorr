    ...
    def peer_register(self, dtls: list) -> str:
        ...
        with open('users.json', 'r+') as f:
            ...
            for i in usrs['users']:
                ...
                t_port_list.insert(tk.END, f'Peer {self.peer_port} -> {dtls[0]} {dtls[1]} registerd')
            ...
            for i in users['users']:
                if int(dtls[0]) == i['uid']:
                    ...
                    if pwd == i['password']:
                        ...
                        t_port_list.insert(tk.END, f'Peer {self.peer_port} {dtls[0]} logged in')
...
class FileOperation:
    ...
    def file_entry(self, file_block: dict) -> str:
        ...
        t_port_list.insert(tk.END, f'Peer {self.peer_port} Uid {self.uid} Savefile name {file_block["FileName"]}')
    ...
    def file_show(self) -> dict:
        ...
        t_port_list.insert(tk.END, f'Peer {self.peer_port} Uid {self.uid} Ask for files')
    ...
    def file_detail(self, name: str) -> dict:
        ...
        t_port_list.insert(tk.END, f'Peer {self.peer_port} Uid {self.uid} Ask for file {name} detail')
    ...
    def peer_chat(self):
        ...
        while True:
            try:
                ...
                t_port_list.insert(tk.END, f'Peer {self.peer_port} unexpectedly disconnected')
                logging.info(f'Peer {self.peer_port} unexpectedly disconnected')
                break

            logging.info(f'Peer {self.peer_port} -> {msg}')
            t_port_list.insert(tk.END, f'Peer {self.peer_port} -> {msg}')
            ...
            if msg == 'register':
                ...
                t_port_list.insert(tk.END, f'Peer {self.peer_port} -> disconnected')
...
logging.basicConfig(filename='tracker.log', level=logging.DEBUG,
                    style='{', format=log_format, filemode='a')
...
def check_tracker_port(*args):
    global tracker_port

    try:
        tracker_port = int(t_port_entry.get())
    except:
        ...
        sec_frame.tkraise()
        t_port_label['text'] = f'Tracker Port {tracker_port}'

        threading.Thread(target=server_start).start()

# ====== Staring server ======
def server_start():
    global tracker_server
    ...
    t_port_list.insert(tk.END, f'Tracker started on {tracker_port}')

    while True:
        conn, addr = tracker_server.accept()
        peer_port = conn.recv(10).decode()

        my_peer_obj = MyPeer(conn, int(peer_port))

        t = threading.Thread(target=my_peer_obj.peer_chat)
        logging.info(f'New Peer {peer_port} with thread {t.name}')
        t_port_list.insert(tk.END, f'New Peer {peer_port} with thread {t.name}')
        t.start()
...
t_port_button = ttk.Button(start_frame, text='Start', command=check_tracker_port)
...
t_port_entry.focus()
wind.bind('<Return>', check_tracker_port)
